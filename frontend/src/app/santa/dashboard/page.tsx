...
const LETTER_COLUMNS = (
  ...
  { field: "location", headerName: "Location", width: 150 },
  {
    ...
    renderCell: (params) => (
      <Box
        sx={{ display: "flex", alignItems: "center", gap: 1.5, height: "100%" }}
      >
        <Box
          component="img"
    ...
    renderCell: (params) => (
      <Button
        ...
        sx={{
          color: "#F8B229",
          textTransform: "none",
          "&:hover": { bgcolor: "rgba(248,178,41,0.1)" },
        }}
      >
        View
      </Button>
    ...
    renderCell: (params) => (
      <Box sx={{ display: "flex", gap: 1 }}>
        <Button
          size="small"
          onClick={() =>
            handleUpdateStock(params.row._id, params.row.stock || 0)
          }
...
export default function SantaDashboard() {
  ...
  const handlePackedToggle = useCallback(
    async (id: string, isPacked: boolean) => {
      try {
        await santaApi.patch(`/letters/${id}/packed`, { isPacked });
        setLetters((prev) =>
          prev.map((l) => (l._id === id ? { ...l, isPacked } : l))
        );
      } catch (error) {
  ...
  const columns = useMemo(
    () =>
      LETTER_COLUMNS(handleViewLetter, handlePackedToggle, handleStatusChange),
    [handleViewLetter, handlePackedToggle, handleStatusChange]
  );
  const inventoryColumns = useMemo(
    () => getInventoryColumns(handleUpdateStock, handleEditGift),
    [handleUpdateStock, handleEditGift]
  );
  ...
  return (
    ...
    >
      ...
      <AppBar
        ...
        color="transparent"
        elevation={0}
        sx={{ pt: 2, bgcolor: "rgba(5, 10, 24, 0.8)" }}
      >
        <Toolbar>
          <Typography
            variant="h5"
        ...
        <Tabs
          ...
          sx={{
            ...
            "& .Mui-selected": { color: "#F8B229" },
            "& .MuiTabs-indicator": { bgcolor: "#F8B229" },
          }}
        >
          <Tab label="Letters" />
          ...
          <Box>
            <Grid container spacing={4}>
              <Grid size={{ xs: 12, md: 4 }}>
                ...
                >
                  <Typography variant="h5" sx={{ mb: 3, color: "#F8B229" }}>
                    Add New Key Gift
                  </Typography>
                  <TextField
                    fullWidth
        ...
        >
          ...
          >
            {selectedLetter?.content ||
              "I've been very good this year! Please bring me my gift!"}
          </Typography>
          ...
          <Box sx={{ borderTop: "2px dashed rgba(0,0,0,0.1)", pt: 2, mt: 4 }}>
            ...
            <Typography variant="body1">
              ...
              {selectedLetter?.gift?.title || "Unknown"}
            </Typography>
          </Box>
      ...
      >
        ...
        >
          <Typography
            variant="h6"
            sx={{ mb: 2, color: "#F8B229", fontWeight: "bold" }}
          >
            ✏️ Edit Gift Details
          </Typography>
